...
<html lang="en">
<head>
    <title>Code coverage report for src/lib/application.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="..\..\index.html">All files</a> / <a href="index.html">src/lib</a> application.ts
    </h1>
    <div class='clearfix'>
...
<span class="cstat-no" title="statement not covered" >            this.logger = new CallbackLogger(&lt;any&gt; logger);</span>
        } else if (logger === 'none') {
...
<span class="cstat-no" title="statement not covered" >            this.logger.diagnostics(result.errors);</span>
            if (this.ignoreCompilerErrors) {
<span class="cstat-no" title="statement not covered" >                this.logger.resetErrors();</span>
<span class="cstat-no" title="statement not covered" >                return result.project;</span>
            } else {
...
&nbsp;
    ...
    public generateDocs(input: ProjectReflection | string[], out: string): boolean {
        const project = input instanceof ProjectReflection ? input : <span class="branch-1 cbranch-no" title="branch not covered" >this.convert(input);</span>
        if (!project) {
...
<span class="cstat-no" title="statement not covered" >            this.logger.error('Documentation could not be generated due to the errors above.');</span>
        } else {
...
&nbsp;
     ...
     */
    public <span class="fstat-no" title="function not covered" >generateJson(</span>input: ProjectReflection | string[], out: string): boolean {
        const project = <span class="cstat-no" title="statement not covered" >input instanceof ProjectReflection ? input : this.convert(input);</span>
        if (!project) {
<span class="cstat-no" title="statement not covered" >            return false;</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        out = Path.resolve(out);</span>
        const eventData = <span class="cstat-no" title="statement not covered" >{ outputDirectory: Path.dirname(out), outputFile: Path.basename(out) };</span>
        const ser = <span class="cstat-no" title="statement not covered" >this.serializer.projectToObject(project, { begin: eventData, end: eventData });</span>
<span class="cstat-no" title="statement not covered" >        writeFile(out, JSON.stringify(ser, null, '\t'), false);</span>
<span class="cstat-no" title="statement not covered" >        this.logger.success('JSON written to %s', out);</span>
&nbsp;
...
&nbsp;
     ...
     */
    public expandInputFiles(inputFiles: string[] = <span class="branch-0 cbranch-no" title="branch not covered" >[])</span>: string[] {
        let files: string[] = [];
&nbsp;
...
&nbsp;
        const supportedFileRegex = this.options.getCompilerOptions().allowJs ? <span class="branch-0 cbranch-no" title="branch not covered" >/\.[tj]sx?$/ </span>: /\.tsx?$/;
...
&nbsp;
     ...
     */
    public <span class="fstat-no" title="function not covered" >toString(</span>) {
<span class="cstat-no" title="statement not covered" >        return [</span>
            '',
