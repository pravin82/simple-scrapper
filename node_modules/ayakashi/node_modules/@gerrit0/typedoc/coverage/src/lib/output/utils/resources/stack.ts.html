...
<html lang="en">
<head>
    <title>Code coverage report for src/lib/output/utils/resources/stack.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="..\..\..\..\..\index.html">All files</a> / <a href="index.html">src/lib/output/utils/resources</a> stack.ts
    </h1>
    <div class='clearfix'>
...
&nbsp;
    private findResources(dir?: string) {
        ...
        let path = this.path;
        if (dir) {
<span class="cstat-no" title="statement not covered" >            path = Path.join(path, dir);</span>
        }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >                this.findResources(dir ? Path.join(dir, fileName) : fileName);</span>
            } else if (resourceRegExp.test(fileName)) {
                const name: string = normalizeName(dir ? <span class="branch-0 cbranch-no" title="branch not covered" >Path.join(dir, fileName) </span>: fileName);
...
&nbsp;
    constructor(resourceClass: ResourceClass&lt;T&gt;, resourceRegExp?: RegExp) {
        ...
        this.resourceRegExp = resourceRegExp || <span class="branch-1 cbranch-no" title="branch not covered" >/.*/;</span>
    }
&nbsp;
    activate(): boolean {
        if (this.isActive) {
...
&nbsp;
<span class="fstat-no" title="function not covered" >    deactivate(</span>): boolean {
        if (!this.isActive) {
<span class="cstat-no" title="statement not covered" >            return false;</span>
        }
<span class="cstat-no" title="statement not covered" >        this.isActive = false;</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >        throw new Error(Util.format('Cannot find resource `%s`.', name));</span>
    }
&nbsp;
...
&nbsp;
    ...
    addOrigin(name: string, path: string, ignoreErrors?: boolean) {
        if (this.isActive) {
<span class="cstat-no" title="statement not covered" >            throw new Error('Cannot add origins while the resource is active.');</span>
        }
&nbsp;
        if (this.hasOrigin(name)) {
<span class="cstat-no" title="statement not covered" >            throw new Error(Util.format('The origin `%s` is already registered.', name));</span>
        }
&nbsp;
        path = Path.resolve(path);
        if (!FS.existsSync(path)) {
            if (!ignoreErrors) {
<span class="cstat-no" title="statement not covered" >                throw new Error(Util.format('The resource path `%s` does not exist.', path));</span>
            }
            return;
        }
&nbsp;
        if (!FS.statSync(path).isDirectory()) {
            if (!ignoreErrors) {
<span class="cstat-no" title="statement not covered" >                throw new Error(Util.format('The resource path `%s` is not a directory.', path));</span>
            }
...
&nbsp;
<span class="fstat-no" title="function not covered" >    removeOrigin(</span>name: string) {
        if (this.isActive) {
<span class="cstat-no" title="statement not covered" >            throw new Error('Cannot remove origins while the resource is active.');</span>
        }
&nbsp;
        let index = <span class="cstat-no" title="statement not covered" >0,</span> count = <span class="cstat-no" title="statement not covered" >this.origins.length;</span>
        while (index &lt; count) {
            const origin = <span class="cstat-no" title="statement not covered" >this.origins[index];</span>
            if (origin.getName() === name) {
<span class="cstat-no" title="statement not covered" >                this.origins.splice(index, 1);</span>
...
&nbsp;
<span class="fstat-no" title="function not covered" >    removeAllOrigins(</span>) {
        if (this.isActive) {
<span class="cstat-no" title="statement not covered" >            throw new Error('Cannot remove origins while the resource is active.');</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        this.origins = [];</span>
    }
}
&nbsp;</pre></td></tr>
</table></pre>
