...
<html lang="en">
<head>
    <title>Code coverage report for src/lib/output/plugins/MarkedPlugin.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="..\..\..\..\index.html">All files</a> / <a href="index.html">src/lib/output/plugins</a> MarkedPlugin.ts
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">64.29% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>36/56</span>
      </div>
      <div class='fl pad1y space-right2'>
...
&nbsp;
        ...
        Handlebars.registerHelper('relativeURL', (url: string) =&gt; url ? this.getRelativeUrl(url) : <span class="branch-1 cbranch-no" title="branch not covered" >url)</span>;
&nbsp;
        Marked.setOptions({
...
<span class="cstat-no" title="statement not covered" >            this.application.logger.warn(error.message);</span>
...
&nbsp;
    ...
    public parseMarkdown(text: string, context: any) {
        if (this.includes) {
<span class="cstat-no" title="statement not covered" >            text = text.replace(this.includePattern, <span class="fstat-no" title="function not covered" >(m</span>atch: string, path: string) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >                path = Path.join(this.includes!, path.trim());</span>
                if (FS.existsSync(path) &amp;&amp; FS.statSync(path).isFile()) {
                    const contents = <span class="cstat-no" title="statement not covered" >FS.readFileSync(path, 'utf-8');</span>
                    if (path.substr(-4).toLocaleLowerCase() === '.hbs') {
                        const template = <span class="cstat-no" title="statement not covered" >Handlebars.compile(contents);</span>
<span class="cstat-no" title="statement not covered" >                        return template(context);</span>
                    } else {
...
<span class="cstat-no" title="statement not covered" >            text = text.replace(this.mediaPattern, <span class="fstat-no" title="function not covered" >(m</span>atch: string, path: string) =&gt; {</span>
                if (FS.existsSync(Path.join(this.mediaDirectory!, path))) {
<span class="cstat-no" title="statement not covered" >                    return this.getRelativeUrl('media') + '/' + path;</span>
                } else {
...
&nbsp;
        ...
        if (this.includeSource) {
            const includes = <span class="cstat-no" title="statement not covered" >Path.resolve(this.includeSource);</span>
            if (FS.existsSync(includes) &amp;&amp; FS.statSync(includes).isDirectory()) {
<span class="cstat-no" title="statement not covered" >                this.includes = includes;</span>
            } else {
<span class="cstat-no" title="statement not covered" >                this.application.logger.warn('Could not find provided includes directory: ' + includes);</span>
            }
        }
&nbsp;
        if (this.mediaSource) {
            const media = <span class="cstat-no" title="statement not covered" >Path.resolve(this.mediaSource);</span>
            if (FS.existsSync(media) &amp;&amp; FS.statSync(media).isDirectory()) {
<span class="cstat-no" title="statement not covered" >                this.mediaDirectory = Path.join(event.outputDirectory, 'media');</span>
<span class="cstat-no" title="statement not covered" >                FS.copySync(media, this.mediaDirectory);</span>
            } else {
<span class="cstat-no" title="statement not covered" >                this.mediaDirectory = undefined;</span>
<span class="cstat-no" title="statement not covered" >                this.application.logger.warn('Could not find provided media directory: ' + media);</span>
            }
        }
    }
&nbsp;
