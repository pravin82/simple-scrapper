...
<html lang="en">
<head>
    <title>Code coverage report for src/lib/converter/factories/signature.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="..\..\..\..\index.html">All files</a> / <a href="index.html">src/lib/converter/factories</a> signature.ts
    </h1>
    <div class='clearfix'>
...
function extractSignatureType(context: Context, node: ts.SignatureDeclaration): Type | undefined {
    ...
    if (node.kind &amp; ts.SyntaxKind.CallSignature || <span class="branch-1 cbranch-no" title="branch not covered" >node.kind &amp; ts.SyntaxKind.CallExpression)</span> {
        try {
            const signature = checker.getSignatureFromDeclaration(node);
            // This is essentially what checker.getReturnTypeOfSignature will do, but doing it ourselves avoids type errors.
            if (!signature) {
<span class="cstat-no" title="statement not covered" >                throw new Error('Failed to retrieve signature for node.');</span>
            }
            return context.converter.convertType(context, node.type, checker.getReturnTypeOfSignature(signature));
        } catch (error) {}
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return context.converter.convertType(context, node.type || node);</span>
}
&nbsp;</pre></td></tr>
</table></pre>
