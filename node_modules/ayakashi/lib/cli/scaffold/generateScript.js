...
function generateScript(directory, name) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const scriptsFolder = path_1.join(directory, "scripts");
        const filePath = path_1.join(scriptsFolder, fileName);
        if (yield exists(filePath)) {
            opLog.error(`script <${name}> already exists in ${filePath}`);
            return;
        }
        opLog.info(`Created <${name}> in ${filePath}`);
        yield mkdirp(scriptsFolder);
...
function getContent() {
    return (`module.exports = async function(input, params) {
