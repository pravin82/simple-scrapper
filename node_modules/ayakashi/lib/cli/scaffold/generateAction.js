...
function generateAction(directory, name) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const actionsFolder = path_1.join(directory, "actions");
        const filePath = path_1.join(actionsFolder, fileName);
        if (yield exists(filePath)) {
            opLog.error(`action <${name}> already exists in ${filePath}`);
            return;
        }
        opLog.info(`Created <${name}> in ${filePath}`);
        yield mkdirp(actionsFolder);
